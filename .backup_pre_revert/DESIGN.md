...
### En-tête applicatif
- Barre fine, pas de bannière dégradée. Marque `ECOSYS` (carré vert + nom en `--ink`), à droite
  le contexte opérationnel (période, nb de points). Filet bas `1px var(--border)`.

### Bandeau de KPI
- Une seule rangée `border: 1px solid var(--border)`, rayon 12px, cellules séparées par un
  filet vertical. Valeur en `--ink` 1.5rem 800 ; la cellule « Points prioritaires » est la
  seule accentuée (valeur rouge + fond `--vert-50`) — c'est le signal d'urgence.

### Carte KPI unitaire (cas particuliers : simulation)
- Fond blanc, `border: 1px solid var(--border)`, rayon 12px, **pas** d'ombre ni de side-stripe.
  label 0.72rem uppercase (gris-bleu), valeur 1.5rem 800 (`--ink`), suffixe 0.9rem.
...
### Cheminement (pipeline IA)
- `stepper()` : 4 étapes en flex, séparées par un chevron, bordure fine, pas de cartes vertes.
...
- `utils/ui.py` expose `entete_app`, `entete_page`, `titre_section`, `bande_kpi`, `carte_kpi`,
  `badge_priorite`, `afficher_figure` (habillage Plotly cohérent), `stepper`, `synthese_operationnelle`
  et `style_table` (Styler pandas).
